...
class Grupos extends CI_Controller {
    ...
    public function index($state = '', $grado = 7, $idGrupo = -1, $error = '') {
        ...
        $titulo = 'Conformar grupos del curso ' . $this->curso->activo_str();
        ...
        if (is_null($grupos))
            ...
            $this->load->view('grupos', array('grado' => $grado, 'idGrupo' => $idGrupo, 'grupos' => $grupos, 'state' => $state, 'error' => $error));
    ...
    public function edit($idGrupo, $grado = 7) {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($insertando)
                $result = $this->grupo->insertar($grado, $nombre, true);
            else
                $result = $this->grupo->actualizar($idGrupo, $grado, $nombre, true);

            if ($result === TRUE) {
                //Redireccionar para eliminar los datos del formulario
                if ($insertando) {
                    $this->session->set_flashdata('success', 'El grupo ha sido agregado con éxito');
                    $this->log->Insertar('Agregó el grupo ' . $grado . '-' . $nombre . ' del curso ' . $this->curso->activo_str());
                    redirect('grupos/edit/new/' . $grado);
                } else {
                    $this->session->set_flashdata('success', 'El grupo fue actualizado con éxito');
                    $this->log->Insertar('Actualizó el grupo ' . $grado . '-' . $nombre . ' del curso ' . $this->curso->activo_str());
                    redirect('grupos/');
                }
    ...
    function delete($idGrupo) {
        ...
        if ($result === TRUE) {
            ...
            $this->log->Insertar('Eliminó el grupo con Id=' . $idGrupo . ' del curso ' . $this->curso->activo_str());
            redirect('grupos/');
            return;
        }
