...
class Colores_piel extends CI_Controller {
    ...
    public function index($state = '', $idColor = -1, $error = '') {
        $colores = $this->color_piel->lista(true);

        $this->load->view('header', array('titulo' => 'Colores de piel'));
        ...
        if (is_null($colores))
            ...
            $this->load->view('su/colores_piel', array('idColor' => $idColor, 'colores' => $colores, 'state' => $state, 'error' => $error));
        }
    ...
    public function edit($idColor) {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($insertando)
                $result = $this->color_piel->insertar($nombre);
            else
                $result = $this->color_piel->actualizar($idColor, $nombre);
            ...
            if ($result === TRUE) {
                ...
                if ($insertando) {
                    $this->log->Insertar('Insertó el color de piel ' . $nombre);
                    redirect('colores_piel/edit/new');
                } else {
                    $this->log->Insertar('Actualizó un color de piel a ' . $nombre);
                    redirect('colores_piel');
                }
    ...
    function delete($idColor) {
        $result = $this->color_piel->eliminar($idColor);

        if ($result === TRUE) {
            $this->session->set_flashdata('success', true);
            $this->log->Insertar('Eliminó un color de piel');
            redirect('colores_piel');
            return;
        }
