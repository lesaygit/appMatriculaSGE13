...
class Colores_pelo extends CI_Controller {
    ...
    public function index($state = '', $idColor = -1, $error = '') {
        $colores = $this->color_pelo->lista(true);

        $this->load->view('header', array('titulo' => 'Colores de pelo'));
        ...
        if (is_null($colores))
            ...
            $this->load->view('su/colores_pelo', array('idColor' => $idColor, 'colores' => $colores, 'state' => $state, 'error' => $error));
        }
    ...
    public function edit($idColor) {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($insertando)
                $result = $this->color_pelo->insertar($nombre);
            else
                $result = $this->color_pelo->actualizar($idColor, $nombre);
            ...
            if ($result === TRUE) {
                ...
                if ($insertando) {
                    $this->log->Insertar('Insertó el color de pelo ' . $nombre);
                    redirect('colores_pelo/edit/new');
                } else {
                    $this->log->Insertar('Actualizó un color de pelo a ' . $nombre);
                    redirect('colores_pelo');
                }
    ...
    function delete($idColor) {
        $result = $this->color_pelo->eliminar($idColor);

        if ($result === TRUE) {
            $this->session->set_flashdata('success', true);
            $this->log->Insertar('Eliminó un color de pelo');
            redirect('colores_pelo');
            return;
        }
