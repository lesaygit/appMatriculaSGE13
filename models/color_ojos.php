...
class Color_ojos extends CI_Model {
/*
    ...
    public function insertar($nombre) {
        ...
        $this->db->trans_start();
        $this->db->insert('colores_ojos', array('Color' => $nombre));
    ...
    public function actualizar($idColor, $nombre) {
        ...
        $this->db->trans_start();
        $this->db->update('colores_ojos', array('Color' => $nombre), array('idColor' => $idColor));
        $this->db->trans_complete();
    ...
    public function eliminar($idColor) {
        $this->db->trans_start();
        $this->db->delete('colores_ojos', array('idColor' => $idColor));
        $this->db->trans_complete();
    ...
    public function lista($checkUse = false) {
        $query = $this->db->query('
            SELECT
                c.idColor, c.Color' .
                ($checkUse ? ', case when exists(select * from estudiantes e where e.color_ojos = c.idColor) then 1 else 0 end as usado ' : ' ') .
           'FROM colores_ojos c
            ORDER BY c.Color');
        if ($query === FALSE)
            return null;
